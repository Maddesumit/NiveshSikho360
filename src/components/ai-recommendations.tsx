...
export default function AiRecommendations() {
  ...
  const fetchRecommendations = useCallback(async () => {
    ...
    try {
      ...
      setError("Could not load AI insights. The API quota may be exceeded. Please try again later.");
    } finally {
      setLoading(false);
    }
    ...
    if (!hasFetched) {
        return (
            <Card className="text-center py-8">
                <p className="text-muted-foreground">Click the "Generate Insights" button to get personalized trade recommendations.</p>
            </Card>
        );
    }
     if (recommendations.length === 0) {
        return (
            <Card className="text-center py-8">
                <p className="text-muted-foreground">No specific recommendations right now. Your portfolio looks balanced!</p>
            </Card>
        );
    }
